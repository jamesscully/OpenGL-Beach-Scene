...
float* OctFace::midpoint(const float a[], const float b[]) {
...
void OctFace::subdivide(float a[], float b[], float c[], int level)
{
	...
	if (level == 0) {
		...
		a1.x = a[0]; a1.y = a[1] + offset; a1.z = a[2];
		a2.x = b[0]; a2.y = b[1] + offset; a2.z = b[2];
		a3.x = c[0]; a3.y = c[1] + offset; a3.z = c[2];
